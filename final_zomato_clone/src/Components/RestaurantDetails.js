...
const openRazorPay=async ()=>{
    try{
        let data ;
       data = await fetch("http://localhost:4040/pay/razorpay", {
            method: "POST",
      ...
      const options = {
        ...
        handler: function (response) {
            var values ={
              ...
              fetch("http://localhost:4040/pay/transaction", {
                method: "POST",
                headers: { 'Content-Type': 'application/json' },
...
const loadScript = (src) => {
    ...
    useEffect(() => {
        fetch(`http://localhost:4040/restaurantDetails/${rName}`,{method:'GET'})
        .then(response=>response.json())
        .then(data=>setRestaurant(data.data))
        

        fetch(`http://localhost:4040/menu/${rName}`,{method:'GET'})
        .then(response=>response.json())
